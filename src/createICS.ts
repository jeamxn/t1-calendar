...
const createICS = (events: Event[]): string => {
  ...
  for (const event of events) {
    ...
    const start = event.allDay ? dayjs(event.startAtAllDay).tz("Asia/Seoul", true) : dayjs(event.startAt).tz("Asia/Seoul", true);
    const end = event.allDay ? dayjs(event.startAtAllDay).tz("Asia/Seoul", true) : event.endAt ? dayjs(event.endAt).tz("Asia/Seoul", true) : start.add(1, "hour");
